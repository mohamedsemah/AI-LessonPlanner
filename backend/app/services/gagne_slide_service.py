...
class GagneEventSlideService:
    """Service for generating comprehensive slides for Gagne's Nine Events of Instruction"""
    ...
    def __init__(self):
        ...
        logger.info("GagneEventSlideService initialized successfully")
    ...
    async def generate_slides_for_all_events(
        ...
        try:
            logger.info("Starting slide generation for all Gagne events")
            ...
            logger.info(f"Generated {total_slides} slides across {len(event_slides)} events")
            ...
            logger.error(f"Error in generate_slides_for_all_events: {str(e)}")
            raise
    ...
    async def _generate_slides_for_event(
        ...
        try:
            ...
            logger.info(f"Generating {slide_count} slides for Event {event_number}: {event_name}")
            ...
            logger.error(f"Error generating slides for event {event.event_number}: {str(e)}")
            ...
            for attempt in range(3):
                try:
                    ...
                    if len(slides_data) != slide_count:
                        logger.warning(f"Generated {len(slides_data)} slides, expected {slide_count}")
                        if attempt < 2:
                            continue
                    ...
                    logger.info(f"Successfully generated {len(slides)} slides for event {event.event_number}")
                    return slides
                    
                except (json.JSONDecodeError, KeyError, ValueError) as e:
                    logger.warning(f"Attempt {attempt + 1} failed for event {event.event_number}: {str(e)}")
                    if attempt < 2:
                        continue
                    else:
                        # Final attempt failed, use fallback
                        logger.warning(f"All attempts failed for event {event.event_number}, using fallback")
                        return self._create_fallback_slides(event, objectives, slide_count)
            
        except Exception as e:
            logger.error(f"Error in _create_ai_generated_slides: {str(e)}")
            return self._create_fallback_slides(event, objectives, slide_count)
